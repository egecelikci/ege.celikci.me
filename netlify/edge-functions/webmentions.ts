 ...
 */
const simpleSanitize = (str: string | undefined,): string => {
  if (!str) return "";
...
const cleanMentions = (entry: any,) => {
  ...
  if (entry.author && entry.author.photo) {
    ...
    // NOTE: In Edge Functions, we might need absolute URL or handle route mapping carefully.
    // Assuming the edge function is mapped to /netlify/edge-functions/webmentions or similar via redirects?
    // Actually, in Edge Functions, the path is determined by netlify.toml config.
    // For now, let's keep the path as is, but ensure the edge function handles it.
    entry.author.photo = `/api/webmentions?avatar=${
      encodeURIComponent(
        entry.author.photo,
      )
    }`;
...
const processMentions = (webmentions: any[],) => {
  const allowedTypes = ["in-reply-to", "mention-of", "like-of", "repost-of",];

  const checkRequiredFields = (entry: any,) => {
    const { author, } = entry;
...
export default async (req: Request, _context: any,) => {
  ...
  const webmentionUrl = `${API_ORIGIN}?per-page=1000&target=${
    encodeURIComponent(
      target,
    )
  }`;
