...
async function verifyPasskey(
  ...
  try {
    ...
    const clientDataJSON = JSON.parse(
      new TextDecoder().decode(base64Decode(assertion.clientDataJSON,),),
    );
...
export default async (req: Request,) => {
  ...
  if (OWNER_TOKEN && authHeader === `Bearer ${OWNER_TOKEN}`) {
    ...
    isAuthorized = await verifyPasskey(
      passkeyHeader,
      "authorize-like-" + mbid,
    );
  }
  ...
  try {
    ...
    if (!lbRes.ok) {
      ...
      console.error("[like] ListenBrainz error:", {
        status: lbRes.status,
        msg: err,
      },);
      return new Response(`ListenBrainz error: ${err}`, {
        status: lbRes.status,
      },);
    }
