...
async function verifyPasskeySignature(assertion: any, expectedChallenge: string): Promise<boolean> {
  ...
  try {
    ...
    const clientDataJSONStr = new TextDecoder().decode(base64Decode(assertion.clientDataJSON));
    ...
    const isAllowedOrigin =
      origin.includes("ege.celikci.me") ||
...
export default async (req: Request) => {
  ...
  try {
    ...
    if (!assertion || !OWNER_PASSKEY_CONFIG) {
       return new Response(JSON.stringify({ success: false, error: "Missing config or assertion" }), { 
         status: 401,
    ...
    if (!isValid) {
      return new Response(JSON.stringify({ success: false, error: "Invalid signature or device mismatch" }), { 
        status: 401,
        headers: { "Content-Type": "application/json" }
      });
    }
