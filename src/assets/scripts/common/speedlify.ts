(function() {
  ...
  class SpeedlifyUrlStore {
    fetches: Record<string, Promise<Response>>;
    responses: Record<string, Promise<any>>;
    ...
    async fetch(speedlifyUrl: string, url: string,) {
  ...
  customElements.define(
    ...
    class extends HTMLElement {
      speedlifyUrl: string | null = null;
      shorthash: string | null = null;
      rawData: string | null = null;
      url: string | null = null;
      urlStore: SpeedlifyUrlStore | null = null;
      ...
      async init() {
        ...
        if (!hash && this.urlStore && this.speedlifyUrl && this.url) {
      ...
      getScoreTemplate(data: any,) {
        let scores = [];
        scores.push(
          `<span title="Performance" class="${
            this.getScoreClass(
              data.lighthouse.performance,
            )
          }">${
            parseInt(String(data.lighthouse.performance * 100,), 10,)
          }</span>`,
        );
        scores.push(
          `<span title="Accessibility" class="${
            this.getScoreClass(
              data.lighthouse.accessibility,
            )
          }">${
            parseInt(String(data.lighthouse.accessibility * 100,), 10,)
          }</span>`,
        );
        scores.push(
          `<span title="Best Practices" class="${
            this.getScoreClass(
              data.lighthouse.bestPractices,
            )
          }">${
            parseInt(String(data.lighthouse.bestPractices * 100,), 10,)
          }</span>`,
        );
        scores.push(
          `<span title="SEO" class="${
            this.getScoreClass(
              data.lighthouse.seo,
            )
          }">${parseInt(String(data.lighthouse.seo * 100,), 10,)}</span>`,
        );
        return scores.join(" ",);
      }
