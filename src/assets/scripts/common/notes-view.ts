...
import { animateGridItems, } from "./grid.ts";
import { initPhotoSwipe, } from "./lightbox.ts";
...
class NotesView {
  ...
  toggleBtns: NodeListOf<HTMLElement> | null = null;
  toggleBg: HTMLElement | null = null;
  ...
  init() {
    ...
    this.toggleBtns = this.toggleContainer.querySelectorAll(
      ".view-toggle__btn",
    );
  ...
  updateToggleVisuals(view: string, animate = true,) {
    if (!this.toggleContainer || !this.toggleBtns || !this.toggleBg) return;
  ...
  applyView(view: string, animate: boolean,) {
